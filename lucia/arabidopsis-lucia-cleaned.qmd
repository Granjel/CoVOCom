---
title: "Arabidopsis analyses – cleaned"
author: "Lucía Martín (cleaned by Rodrigo & ChatGPT)"
date: "2025-09-06"
format:
  html:
    toc: true
...
suppressPackageStartupMessages({
  ...
  # ordination and permutations
  library(vegan)
  library(permute)

  # additional tools
  library(lattice)
  library(reshape) # only if legacy reshaping is required
})
```
...
> the code below reads the original datasets as in the source analysis. if a file is missing, rendering will stop with a clear message.

```{r}
# helper to assert files exist before reading
...
# common files referenced in the original Rmd (add here if needed)
candidate_files <- c(
  "damage.emitters.csv",
  "damage.receivers.csv",
  "VOCs.csv"
)

# report availability
data.frame(file = candidate_files, exists = file.exists(candidate_files)) |>
...
the original analysis code follows, organized as in the source file. comments have been added where appropriate. outputs are shown inline.
...
### generalized mixed models (glmmTMB)

### post-hoc comparisons (lsmeans)
...
write.xlsx(lsmeanse, "lsmeans emitters.pop.xlsx")
...
data <- read.csv(file = "Damage2.csv", h = T, sep = ";", na.strings = "NA")
