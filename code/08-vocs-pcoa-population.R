...
# review and see that it matches the PCoA for treatments
...
# load the subset of data used for population-level PERMANOVA (created in code/07-vocs-permanova.R)
vocs_subset <- read.csv("tables/permanova/data-subsets/vocs-subset-all.csv") # currently, the subset is "All" but it could be changed to a specific type of VOCs if desired

# PCoA -------------------------------------------------------------------

# use capscale to do PCoA
vocs_pcoa_population <- capscale(
  compounds_bray ~ 1 + Condition(vocs_subset$treatment),
  data = vocs_subset,
  comm = vocs_subset %>% dplyr::select(voc1:voc17)
)
...
envfit_population <- envfit(
  ...
  vocs_subset %>% dplyr::select(code, population, voc1:voc17)
)
...
vocs_pcoa_treatment <- capscale(
  dplyr::select(vocs, starts_with("voc")) ~ 1 + Condition(treatment),
  data = vocs,
  distance = "bray"
)
