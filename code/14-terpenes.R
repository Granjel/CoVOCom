...
write.csv(
  as.data.frame(emm_terpenes$emmeans),
  "tables/emm-terpenes-emmeans.csv",
  row.names = FALSE
)

# save pairwise contrasts to a CSV file
write.csv(
  as.data.frame(emm_terpenes$contrasts),
  "tables/emm-terpenes-contrasts.csv",
  row.names = FALSE
)

# compact letter display (CLD) for EMMs for treatment within population
cld_terpenes <- cld(emm_terpenes, Letters = TRUE)

# p-values for pairwise comparisons
pvalue_terpenes <- summary(emm_terpenes$contrasts)$p.value
...
# plot Long chain terpenes emissions by treatment and population
p_terpenes <-
  ggplot(
    vocs_terpenes,
    aes(x = population, y = Terpenes, fill = treatment)
  ) +
...
stars_terpenes <- vocs_terpenes %>%
  dplyr::distinct(population) %>%
  dplyr::mutate(
    y = quantile(vocs_terpenes$Terpenes, c(0.925)) * 1.05,
    label = dplyr::case_when(
      pvalue_terpenes < 0.001 ~ "***",
      pvalue_terpenes < 0.01 ~ "**",
      pvalue_terpenes < 0.05 ~ "*",
      TRUE ~ ""
    )
  )
